...
class WhiStress(PreTrainedModel):
    ...
    def __init__(
        ...
        self.whisper_model = WhisperForConditionalGeneration.from_pretrained(
            self.whisper_backbone_name, use_auth_token=use_auth_token
        ).eval()
        self.processor = WhisperProcessor.from_pretrained(self.whisper_backbone_name, use_auth_token=use_auth_token)
    ...
    def train(self, mode: Optional[bool] = True):
        # freeze whisper and train only classifier
    ...
    def forward(
        ...
        # Extract the hidden states of the decoder
    ...
    def generate(
        ...
        **generate_kwargs,
        ...
        # Extract the hidden states of the encoder
        layer_for_head_hidden_states = backbone_outputs.encoder_hidden_states[
            self.layer_for_head
        ].to(device)
        # Pass the decoder last hidden layers through the new head (decoder_block + lin cls)
