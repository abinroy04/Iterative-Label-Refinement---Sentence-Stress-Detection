...
class DSProcessor:
    ...
    def load_intonation_dataset(self):
        ...
        If max_samples is specified, only loads that many samples from each split.
        ...
        if self.max_samples is not None:
            print(f"Loading only the first {self.max_samples} samples from each split")
            for split in intonation_dataset:
                intonation_dataset[split] = intonation_dataset[split].select(range(min(self.max_samples, len(intonation_dataset[split]))))
    ...
    def emphasized_tokens(self, example, 
        ...
        # Initialize concatenated_values
        concatenated_values = []

        # Handle nested dictionary structure in ITA-word-stress dataset
        if isinstance(example[emphasis_indices_column_name], dict):
            # Use the indices field from the nested dictionary
            indices = example[emphasis_indices_column_name]["indices"]
            try:
                concatenated_values = [
                    item for elem in indices for item in curr_values[elem]
                ]
            except IndexError:
                # Handle error case with graceful fallback
                print(f"Warning: Mismatched indices for sample {example.get('sentence_index', 'unknown')}")
                concatenated_values = []
    
        # Handle list of indices (TinyStress format)
        else:
            # Check if we have valid indices before using them
            emphasis_indices = example[emphasis_indices_column_name]
            if len(emphasis_indices) > 0:
                # Check if the largest index is within range
                try:    
                    max_index = max(emphasis_indices)
                    if max_index < len(curr_values):
                        concatenated_values = [
                            item for elem in emphasis_indices for item in curr_values[elem]
                        ]
                    else:
                        print(f"Warning: Index {max_index} out of range for sample {example.get('sentence_index', 'unknown')}")
                except (ValueError, IndexError):
                    print(f"Warning: Invalid indices for sample {example.get('sentence_index', 'unknown')}")

        # Create binary vector for emphasized tokens
        j = 0
        emphasized_words = []
    ...
    def aligned_whisper_transcriptions(self, example, model):
        ...
        # We always use the ground truth transcription but still generate Whisper transcription for comparison
        ...
        # Store both transcriptions, but we'll use ground truth for alignment
        example['whisper_transcription'] = transcription_whisper
        
        # Debug - log mismatches :D
        ...
        # Always use ground truth for alignment
        example['aligned_whisper_transcriptions'] = example['transcription']
    ...
    def preprocess(self, 
        ...
        proccess_methods = {
            ...
            # "filter_misaligned_samples": lambda example: self.filter_misaligned_samples(example, transcription_column_name), - Remove filtering to use GT incase of mismatch with Whisper transcription
    ...
    def get_train_dataset(self, 
        ...
        if columns_to_remove:
            ...
            available_columns = set(preproc_intonation_dataset["train"].column_names)
            # Filter columns_to_remove to only include columns that exist
            columns_to_remove_filtered = [col for col in columns_to_remove if col in available_columns]
            # Remove the filtered columns
            if columns_to_remove_filtered:
                preproc_intonation_dataset = preproc_intonation_dataset.remove_columns(columns_to_remove_filtered)
